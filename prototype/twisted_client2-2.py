...
class UserProtocol(LineOnlyReceiver):
    ...
    # when user program sends a request for position data
    def lineReceived(self, line):
        if len(line) == 1:
            if line in options:
                self.factory.client.send_msg(line)
            #TODO: handle else conditions
...
if __name__ == 'main':
    ...
    if(host is not None):
        ...
        # # for taking in input from user program
        #
        # user_server = endpoints.TCP4ServerEndpoint(reactor, 45002)
        # user_server.listen(UserFactory)
        #
        # # for sending and receiving msgs from twisted server
        # motive_ep = endpoints.TCP4ClientEndpoint(reactor, host, defaultServerPort, timeout=30)
        # motive_ep.connect(SocketClientFactory)
